...
export class PractiseQuestionComponent implements OnInit {
  ...
  constructor(private _location:Location,private route: ActivatedRoute,private router:Router, private student: StudentService,private snackbar:MatSnackBar) { }
  ...
  ngOnInit(): void {
    ...
    this.sharableUrl="http://35.226.197.242/#/share-preview-page/"+this.qDetailsId;
    ...
    this.student.checkIfWhishListed(this.qDetailsId,this.userId)
    .subscribe(
      data=>
      {
        console.log(data);
    ...
    this.student.getLikesDislikes(this.qDetailsId,this.userId,this.usertype)
    .subscribe(
      data=>
      {
        console.log(data);
    ...
    this.student.getQuestionById(this.qDetailsId).subscribe(
      res => {
        ...
        this.actualCorrectAnswers = this.qDetails.answersDTO.correctOption.split(",");
        console.log("question retrived form db: ", this.actualCorrectAnswers);
      },
      err => console.log("Error occured while retriving quesiton from db: ", err)
    )
  }
  ...
  Like(){
    ...
    this.student.postLike(this.sendLike)
    .subscribe(
      data=>
      {
        console.log(data);
  ...
  disLike(){
    ...
    this.student.postDisLike(this.sendDisLike)
    .subscribe(
      data=>
      {
        console.log(data);
  ...
  addToList(){
    ...
    this.student.addToWishList(this.addFavPayload)
    .subscribe(
      data=>
      {
        console.log(data);
  ...
  questiondata(){
    this.student.getQuestionData(this.qDetailsId)
    .subscribe(
      (data : any)=>
      {
  ...
  deleteFromList(){
    ...
    this.addFavPayload.qid=this.qDetailsId;
    this.addFavPayload.userId=sessionStorage.getItem("userid");
    this.student.removeFromWishList(this.addFavPayload)
    .subscribe(
      data=>
      {
        console.log(data);
  ...
  validateAndProcess() {
    ...
    this.student.submitSolution(payload).subscribe(
      res => {
        ...
        this.snackbar.open('Submitted succesfully','close',{duration:3000});
  ...
  postTag(name,year){
    ...
    console.log(tagData);
    this.student.postTagForQuestion(tagData)
    .subscribe(
      (data:any)=>
      {
        console.log(data);
